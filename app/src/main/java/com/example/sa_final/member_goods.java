...
public class member_goods extends AppCompatActivity {
    ...
    private void getData() {
        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                LinearLayout lle = new LinearLayout(member_goods.this);
                ...
                for (int j=0;j<i[0];j++) {
                    getId1();
                    Log.e("id",getId1());
                    Log.e("memberid",Sid[j]);
                    if (Sid[j].trim() .matches( getId1().trim())) {


                        ll[j] = new LinearLayout(member_goods.this);
                        ll[j].setOrientation(LinearLayout.HORIZONTAL);
                        ll[j].setGravity(Gravity.CENTER_VERTICAL);
                        lle.addView(ll[j]);
                        imageView[j] = new ImageView(member_goods.this);
                        imageView[j].setMinimumWidth(250);
                        imageView[j].setMinimumHeight(250);
                        if (!isDestroy((Activity) member_goods.this)) {
                            Glide.with(member_goods.this).load("https://firebasestorage.googleapis.com/v0/b/cycusa-71b70.appspot.com/o/GImages%2F" + Simageid[j] + "?alt=media").into(imageView[j]);
                        }
                        ...
                        tvname[j] = new TextView(member_goods.this);
                        tvname[j].setText("  " + Sname[j]);
                        tvname[j].setTextSize(TypedValue.COMPLEX_UNIT_DIP, 20);
                        tvname[j].setWidth(200);
                        ll[j].addView(tvname[j]);

                        tvmoney[j] = new TextView(member_goods.this);
                        ...
                        btn[j] = new Button(member_goods.this);
                        btn[j].setText("查看");
                        btn[j].setId(j);
                        ...
                        btn[j].setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                intent.setClass(member_goods.this, member_good.class);
